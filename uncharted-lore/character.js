...
const character = UC_CHARACTERS.find(c => c.id === requestedId);
...
if (character) {
  ...
  const game = UC_GAMES.find(g => g.id === character.game);
  const isRival = character.role === 'Antagonist';

  // point the header's back link at this character's own expedition page
  ...
  view.innerHTML = `
    <div class="file-scene ${isRival ? 'rival' : ''}">
